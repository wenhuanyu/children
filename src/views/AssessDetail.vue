                    ...
                    <div class="print-body">
                        <div class="first ">
                            ...
                            <div class="first-group">
                                ...
                                <div class="row">
                                    <span>患者来源</span>
                                    <p>{{ infoData.patient_info && infoData.patient_info.district_id_text }}</p>
                                </div>
                                <div class="row">
                                    <span>完成时间</span>
                        ...
                        <div class="flex" style="border-bottom: 1px solid #e6e6e6; padding: 10px 25px;" v-if="infoData.calc_name == 'Gesell'">
                            <div style="width: 50%;border-right: 1px solid #e6e6e6; padding-left: 25px;">
                                <div>
                                    <span class="sc1">生产史&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;胎次</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.taici }}</span>
                                    <span class="sc1" style="padding-left: 30px;">产次</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.chanci }}</span>
                                    <span class="sc2" style="padding-left: 25px;">（{{ infoData.patient_info && infoData.patient_info.scs_radio_text }}）</span>
                                </div>
                                <div>
                                    <span class="sc1">出生体重</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.birth_weight }}kg</span>
                                </div>
                                <div>
                                    <span class="sc1">现在&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;身长</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.now_height }}cm</span>
                                    <span class="sc1" style="padding-left: 30px;">体重</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.now_weight }}kg</span>
                                    <span class="sc1" style="padding-left: 25px;">头围</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.now_head }}cm</span>
                                </div>
                            </div>
                            <div style="width: 50%;padding-left: 45px;">
                                <div>
                                    <span class="sc1">窒息</span>
                                    <span class="sc2">（{{ infoData.patient_info && infoData.patient_info.zhixi_text }}）</span>
                                    <span class="sc1" style="padding-left: 15px;">持续时间</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.zhixi_time }}分钟</span>
                                </div>
                                <div>
                                    <span class="sc1">畸形</span>
                                    <span class="sc2">{{ infoData.patient_info && infoData.patient_info.malformation }}</span>
                                </div>
                                <div>
                                    <span class="sc1">有无抽风</span>
                                    <span class="sc2" style="padding-left: 5px;">（{{ infoData.patient_info && infoData.patient_info.choufeng }}抽风）</span>
                                    <span class="sc1" style="padding-left: 10px;">有无卤门</span>
                                    <span class="sc2" style="padding-left: 5px;">（{{ infoData.patient_info && infoData.patient_info.lumen }}卤门）</span>
                                    <span class="sc1" style="padding-left: 10px;">有无黄疸</span>
                                    <span class="sc2" style="padding-left: 5px;">（{{ infoData.patient_info && infoData.patient_info.huangdan }}黄疸）</span>
                                </div>
                            </div>
                        </div>
                        <div v-if="infoData.calc_name === 'DPT'" style="border-bottom: 1px solid #e6e6e6" :class="printData[0].value ? '' : 'print-none'">
                            <div class="flex">
                                <div style="width: 60%; padding: 15px 25px;">
                                    <img :src=tuhua style="width: 100%; height: auto;margin-top: 100px;">
                                </div>
                                <div style="width: 40%">
                                    <div class="flex" style="text-align: center; font-size: 18px; line-height: 54px; font-weight: bold; border-bottom: 1px solid #e6e6e6;">
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">序号</div>
                                        <div style="width: 34%;border-left: 1px solid #e6e6e6;">评测部位</div>
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">满分</div>
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">得分</div>
                                    </div>
                                    <div class="flex" style="text-align: center; font-size: 16px; line-height: 36px; font-weight: bold; border-bottom: 1px solid #e6e6e6;"
                                        v-for="(item,index) in dafen" :key="index" v-if="item.title !=='合计' ">
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">{{ item.num }}</div>
                                        <div style="width: 34%;border-left: 1px solid #e6e6e6;">{{ item.title }}</div>
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">{{ item.total_score }}</div>
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">{{ item.get_score }}</div>
                                    </div>
                                    <div class="flex" style="text-align: center; font-size: 18px; line-height: 54px; font-weight: bold; ">
                                        <div style="width: 56%;border-left: 1px solid #e6e6e6;">总计</div>
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">{{ dafen[dafen.length-1].total_score }}</div>
                                        <div style="width: 22%;border-left: 1px solid #e6e6e6;">{{ dafen[dafen.length-1].get_score }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="print-row" v-if="printShow && infoData.calc_name !== 'FMFM'" >
                            <div class="title"></div>
                        ...
                        <template v-if="infoData.calc_name !== 'MMPICT'">
                            ...
                            <div class="print-row" :class="printData[2].value ? '' : 'print-none'" v-else>
                                ...
                                <div style="display: flex; flex-wrap: wrap; font-size: 22px;margin-bottom: 30px; "  v-if="infoData.calc_name === 'FMFM'">
                                    <div>
                                        <div style="display: flex;margin-bottom: 20px">
                                            <div style="width: 340px;margin-right: 30px;margin-left: 50px;">项目内容</div>
                                            <div>得分</div>
                                        </div>
                                        <div style="margin-right: 100px;" v-if="infoData.calc_name === 'FMFM'" v-for="(item,index) in extend_list.slice(0,31)" :key="index">
                                            <div style="display: flex;margin-bottom: 20px">
                                                <div style="width: 400px;margin-right: 30px">{{ item.title }}</div>
                                                <div>{{item.score}}</div>
                                            </div>
                                        </div>
                                    </div>
                                   <div>
                                       ...
                                       <div v-if="infoData.calc_name === 'FMFM'" v-for="(item,index) in extend_list.slice(31,61)" :key="index">
                                           <div style="display: flex;margin-bottom: 20px">
                                               <div style="width: 400px;margin-right: 30px">{{ item.title }}</div>
                                               <div>{{item.score}}</div>
                                           </div>
                                ...
                                <div style="margin: 20px 0" v-if="Leisure" class="jiance">
                                    <p v-if="infoData.calc_name !== 'DPT'" style="margin-top: 5px;">答案详情 :</p>
                                    <p v-if="infoData.calc_name !== 'DPT'" v-for="item in infoData.answer_detail" :key="index">
...
<!--                                        <span style="display: inline-block;width: 50px"></span>-->
                                ...
                                <div  v-if="infoData.calc_name === 'FMFM'" style="margin-bottom: 20px;font-size: 22px">{{infoData.extend.diagnosis_result}}</div>
                            </div>
                            ...
                            <div class="print-row" :class="printData[3].value ? '' : 'print-none'" v-if="infoData.resting_period == 1">
                                ...
                                <div class="physical-signs-wrap">
                                    <div class="physical-signs" v-for="item in physiologyList">
                                        ...
                                        <div style="width: 40%; color: #666666;font-size: 20px;margin-right: 5px;">{{item.desc}}</div>
                ...
                <div style="width: 75%;">
                    <el-form label-width="120px" style="max-height: unset;">
                        ...
                        <el-form-item label="患者来源" prop="district_id">
                            <el-select v-model="xiugai.district_id_text" style="width: 100%;">
        ...
        async getInfo() {
            await this.$axios.post('api/exam/report', this.$qs.stringify(this.detailForm)).then(res => {
                if (res.data.code === 1) {
                    ...
                    this.extend_list =  res.data.data.extend.list
                    ...
                    if(res.data.data.calc_name === 'DPT') {
                        this.printShow = false
                        this.tuhua = res.data.data.image
                        this.dafen = res.data.data.answer_detail
                    }
...
.charts-16pf-line {
    position: absolute;
    background-image: linear-gradient(to right, #20BE18, #b293fb);
    height: 1px;
    z-index: 1;
}
