        ...
        <div class="sheet">
            <div class="sheet-item">
                ...
                <div class="user-info">
                    ...
                    <div class="user-item">
                        <p>患者来源：</p>
                        <span>{{ patientInfo.district_id_text }}</span>
                    </div>
                    <div class="user-item">
                        <p>入院时间：</p>
...
export default {
    ...
    methods: {
        async getInfo() {
            ...
            await this.$axios.post('term/index/user', this.$qs.stringify(params)).then(res => {
                if (res.data.code === 1) {
                    ...
                    this.test_paper_id = res.data.data.exam.test_paper_id
                    console.log('this.test_paper_id',this.test_paper_id)
                    let obj = {
        ...
        toAnswer() {
            console.log('this.practiceType',this.practiceType)
            switch (this.practiceType) {
                // 评测
                case 1:
                    if (this.$route.query.test_paper_id) {
                        if (this.$route.query.test_paper_id == 175) {
                            this.$router.push({
                                name: 'TerminalHuaBan',
                                query: {
                                    patient_id:this.patient_id,
